...
# 2. navigate on your machine to the folder where you wish to connect TaxPhlAn, e.g. /home/user/TaxPhlAn/
...
# 6. this WORKDIR will be a connection between your local machine and the Docker to exchange files if required
# 7. finally, to go inside the Docker image, run: 'docker exec -e COLUMNS="`tput cols`" -e LINES="`tput lines`" -it < CONTAINER ID > /bin/bash'
